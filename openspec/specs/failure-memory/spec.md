...
## Purpose
The failure-memory capability provides structured, append-only, write-only persistence of per-iteration failure entries with a stable v1 schema and on-disk format. Every failed iteration of the solve loop emits one entry to a persistent store and to a run-workdir mirror, so a human can read what went wrong from an entry alone, and so future cross-run retrieval has a real corpus.
